...
# -----------------------------
# 1) src/vault_linker/config.py (add force + clear_cache)
# -----------------------------
...
class BuildConfig:
    ...
    dry_run: bool = False
    force: bool = False         # ignore cache; reprocess all
    clear_cache: bool = False   # wipe cache state at start
PY

# -----------------------------
# 2) src/vault_linker/stages/cache.py (add clear)
# -----------------------------
cat > src/vault_linker/stages/cache.py <<'PY'
from __future__ import annotations
...
# -----------------------------
# 3) src/vault_linker/stages/infer_tags.py (remove bad backslash warning)
# -----------------------------
...
# -----------------------------
# 4) src/vault_linker/pipeline/build.py (respect force/clear_cache, better skip reason)
# -----------------------------
...
# -----------------------------
# 5) src/vault_linker/cli.py (add flags)
# -----------------------------
...
def main(argv=None) -> int:
    ...
    stats = build(cfg)
    log.info(
        "done: total=%d processed=%d skipped_cache=%d skipped_other=%d yaml_errors=%d learned_tags=%d tags_file=%s",
        stats.total, stats.processed, stats.skipped_cache, stats.skipped_other, stats.yaml_errors, stats.learned_tags, cfg.tags_file
    )
    return 0
PY

echo "OK: added --force and --clear-cache"
